...
```mermaid
graph TD
    SOIL[Soil Science] <-->|Mineral-organic interactions| GEO[Geochemistry]
    SOIL <-->|Rhizosphere processes| PLANT[Plant Science]
    SOIL <-->|Microbial decomposition| MICRO[Microbiology]
    PLANT <-->|Mycorrhizal symbiosis| MICRO
    ENV[Environmental Chemistry] <-->|Contaminant-soil interactions| SOIL
    ENV <-->|Bioremediation| MICRO
    CLIMATE[Climate Science] <-->|Carbon cycling| SOIL
    CLIMATE <-->|Atmospheric chemistry| ENV
    STRUCT[Structural Biology] <-->|Enzyme mechanisms| MICRO
